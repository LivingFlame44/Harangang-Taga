...
public class Defender : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            AnimateStartPos();
            transform.position = Vector2.MoveTowards(this.transform.position, new Vector2(startPos, this.transform.position.y), speed * Time.deltaTime);
        }
    ...
    public void CheckFace()
    {
        if(closestEnemy != null)
        {
            ...
            else
            {
                if (faceBack == false)
                {                  
                    animator.Play("Defender_Back_Idle");
                    faceBack = true;
